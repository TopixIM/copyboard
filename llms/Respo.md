...
calcit analyze call-graph --root app.main/main! --ns-prefix app. --include-core --max-depth 5 --format json

# Call count statistics
calcit analyze count-calls
calcit analyze count-calls --root app.main/main! --ns-prefix app. --include-core --format json --sort count
```
...
**CLI 操作注意**:
在使用 `calcit tree replace` 修改字符串时，推荐使用 `|` 前缀或 `--json-leaf` 确保类型正确。

```bash
# 推荐：使用 | 前缀
calcit tree replace ... -e "|Get Started"

# 推荐：使用 --json-leaf 明确指定为叶子节点
calcit tree replace ... -e '"Get Started"' --json-leaf
```
